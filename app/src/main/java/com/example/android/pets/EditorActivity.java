...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void showDeleteConfirmationDialog() {
        ...
        // for the  and negative buttons on the dialog.
    ...
    private void deletePet() {
        if (currentPetUri != null) {
            int number = getContentResolver().delete(currentPetUri, null, null);
            if (number == 0)
    ...
    private void savePet(){
        ...
        int gender=mGender;
        int weight;
        if (TextUtils.isEmpty(mWeightEditText.getText().toString().trim()))weight=0;
        else weight=Integer.parseInt(mWeightEditText.getText().toString().trim());
        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        if (TextUtils.isEmpty(name)&&TextUtils.isEmpty(breed)&&weight==0&&gender==0){
            Toast.makeText(this,getString(R.string.empty_editor),Toast.LENGTH_SHORT).show();
        }
...
//          Create a new map of values, where column names are the keys
            ...
            if (currentPetUri == null) {
                ...
                int number = getContentResolver().update(currentPetUri, values, null, null);
                if (number == 1) {
                    // If the new content URI is null, then there was an error with insertion.
